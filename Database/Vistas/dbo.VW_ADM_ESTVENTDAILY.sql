...
FROM     (SELECT F.CODSUCU, FORMAT(F.FECHAE, 'yyyy-MM-dd') PERIODO, SUM(IIF(F.TIPOFAC = 'A', 1, 0)) NROFACTS, SUM(IIF(F.TIPOFAC = 'A', F.MTOTOTAL, 0)) MTOFACTS, SUM(IIF(F.TIPOFAC = 'B', 1, 0)) NRODEVOL, SUM(IIF(F.TIPOFAC = 'B', 
                  ...
                  GROUP BY F.CODSUCU, FORMAT(F.FECHAE, 'yyyy-MM-dd')) R
