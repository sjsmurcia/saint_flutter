...
| Catalogos | Productos, clientes, empresas/sucursales con paginacion | Clientes ya expuestos en `Saint.Catalog.Api` (GET paginado, GET por codigo, POST upsert). Productos y companias pendientes.
...
| Catalogo | `GET /catalog/clients`, `GET /catalog/clients/{code}`, `POST /catalog/clients` (completado). `GET /catalog/products` y `GET /catalog/companies` pendientes con filtros `tenantId`, `page`, `pageSize`, `updatedAfter`.|
